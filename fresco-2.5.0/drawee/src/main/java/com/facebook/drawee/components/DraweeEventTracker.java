...
public class DraweeEventTracker {
  ...
  private final Queue<DraweeEventTracker.Event> mEventQueue =  new ArrayBlockingQueue<>(MAX_EVENTS_TO_TRACK);
  ...
  public void recordEvent(DraweeEventTracker.Event event) {
    if (!sEnabled) {
      return;
    }
