...
public class ScalingUtilsTest {
  ...
  private void test(float scaleX, float scaleY, float translateX, float translateY, int childWidth, int childHeight, ScaleType scaleType) {
    test(scaleX, scaleY, translateX, translateY, childWidth, childHeight, 0.5f, 0.5f, scaleType);
  }

  private void test(float scaleX, float scaleY, float translateX, float translateY, int childWidth, int childHeight, float focusX, float focusY, ScaleType scaleType) {
    ...
    mActualMatrix.reset();
    scaleType.getTransform(mActualMatrix, mParentBounds, childWidth, childHeight, focusX, focusY);

    AndroidGraphicsTestUtils.assertEquals(mExpectedMatrix, mActualMatrix);
  }

  private static Matrix createTransform(Matrix transform, float scaleX, float scaleY, float translateX, float translateY)
  {
