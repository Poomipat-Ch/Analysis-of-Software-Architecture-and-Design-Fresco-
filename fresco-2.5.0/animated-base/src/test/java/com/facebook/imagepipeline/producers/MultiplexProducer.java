...
public abstract class MultiplexProducer<K, T extends Closeable> implements Producer<> {
  /**
  ...
  class Multiplexer {
    ...
    @GuardedBy("Multiplexer.this")
    @Nullable
    private MultiplexProducer.Multiplexer.ForwardingConsumer mForwardingConsumer;
     ...
     * to clean up.
     */
    private void startInputProducerIfHasAttachedConsumers(com.facebook.common.util.TriState startedAsPrefetch) {
    ...
    public void onFailure(final MultiplexProducer.Multiplexer.ForwardingConsumer consumer, final Throwable t) {
    ...
    public void onNextResult(final MultiplexProducer.Multiplexer.ForwardingConsumer consumer, @Nullable final T closeableObject, @Consumer.Status final int status) {
      Iterator<Pair<Consumer<T>, ProducerContext>> iterator;
      final int size;
      synchronized (Multiplexer.this) {
        // check for late callbacks
    ...
    public void onCancelled(final MultiplexProducer.Multiplexer.ForwardingConsumer forwardingConsumer) {
      synchronized (Multiplexer.this) {
        // check for late callbacks
    ...
    public void onProgressUpdate(MultiplexProducer.Multiplexer.ForwardingConsumer forwardingConsumer, float progress) {
      Iterator<Pair<Consumer<T>, ProducerContext>> iterator;
      synchronized (Multiplexer.this) {
        // check for late callbacks
        if (mForwardingConsumer != forwardingConsumer) {
  ...
  @GuardedBy("this")
  @VisibleForTesting
  final Map<K, MultiplexProducer.Multiplexer> mMultiplexers;
  ...
  @Nullable
  protected synchronized MultiplexProducer.Multiplexer getExistingMultiplexer(K key) {
    return mMultiplexers.get(key);
  }

  private synchronized MultiplexProducer.Multiplexer createAndPutNewMultiplexer(K key) {
    Multiplexer multiplexer = new Multiplexer(key);
    mMultiplexers.put(key, multiplexer);
    return multiplexer;
  }

  protected synchronized void removeMultiplexer(K key, MultiplexProducer.Multiplexer multiplexer) {
