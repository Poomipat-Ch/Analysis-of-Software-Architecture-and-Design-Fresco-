...
public class JobSchedulerTest {
  ...
  private static class TestJobRunnable implements JobScheduler.JobRunnable {
    private static class Job {
      public final com.facebook.imagepipeline.image.EncodedImage encodedImage;

      @Consumer.Status
      public final int status;

      public Job(com.facebook.imagepipeline.image.EncodedImage encodedImage, @Consumer.Status int status) {
    ...
    @Override
    public void run(com.facebook.imagepipeline.image.EncodedImage encodedImage, @Consumer.Status int status) {
      running.set(true);
      try {
  ...
  private com.facebook.imagepipeline.testing.FakeClock mFakeClockForTime;
  ...
  private com.facebook.imagepipeline.image.EncodedImage fakeEncodedImage() {
  ...
  private static void assertJobsEqual(JobSchedulerTest.TestJobRunnable.Job job, com.facebook.imagepipeline.image.EncodedImage encodedImage, @Consumer.Status int status)
  {
    assertReferencesEqual(encodedImage, job.encodedImage);
    assertEquals(status, job.status);
  }

  private static void assertReferencesEqual(com.facebook.imagepipeline.image.EncodedImage expected, com.facebook.imagepipeline.image.EncodedImage actual)
  {
    if (expected == null) {
      assertNull(actual);
    } else {
