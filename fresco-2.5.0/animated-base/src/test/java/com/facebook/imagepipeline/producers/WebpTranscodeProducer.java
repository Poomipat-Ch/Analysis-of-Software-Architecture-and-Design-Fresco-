...
public class WebpTranscodeProducer implements Producer<> {
  ...
  public WebpTranscodeProducer(Executor executor, com.facebook.common.memory.PooledByteBufferFactory pooledByteBufferFactory, Producer<EncodedImage> inputProducer) {
  ...
  private class WebpTranscodeConsumer extends DelegatingConsumer<, > {
    ...
    private com.facebook.common.util.TriState mShouldTranscodeWhenFinished;
  ...
  private void transcodeLastResult(final com.facebook.imagepipeline.image.EncodedImage originalResult, final Consumer<EncodedImage> consumer, final ProducerContext producerContext) {
  ...
  private static com.facebook.common.util.TriState shouldTranscode(final com.facebook.imagepipeline.image.EncodedImage encodedImage)
  {
  ...
  private static void doTranscode(final com.facebook.imagepipeline.image.EncodedImage encodedImage, final com.facebook.common.memory.PooledByteBufferOutputStream outputStream) throws Exception
  {
    InputStream is = Preconditions.checkNotNull(encodedImage.getInputStream());
    ImageFormat imageFormat = ImageFormatChecker.getImageFormat_WrapIOException(is);
