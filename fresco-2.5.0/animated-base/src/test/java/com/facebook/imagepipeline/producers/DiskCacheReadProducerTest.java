...
public class DiskCacheReadProducerTest {
  ...
  private final com.facebook.imagepipeline.cache.BufferedDiskCache mDefaultBufferedDiskCache =  mock(BufferedDiskCache.class);

  private final com.facebook.imagepipeline.cache.BufferedDiskCache mSmallImageBufferedDiskCache =  mock(BufferedDiskCache.class);
  ...
  private com.facebook.common.memory.PooledByteBuffer mIntermediatePooledByteBuffer;

  private com.facebook.common.memory.PooledByteBuffer mFinalPooledByteBuffer;
  ...
  private void setupDiskCacheGetWait(com.facebook.imagepipeline.cache.BufferedDiskCache bufferedDiskCache) {
  ...
  private void setupDiskCacheGetSuccess(com.facebook.imagepipeline.cache.BufferedDiskCache bufferedDiskCache) {
    when(bufferedDiskCache.get(eq(mCacheKey), any(AtomicBoolean.class)))
        .thenReturn(Task.forResult(mFinalEncodedImage));
  }

  private void setupDiskCacheGetNotFound(com.facebook.imagepipeline.cache.BufferedDiskCache bufferedDiskCache) {
    when(bufferedDiskCache.get(eq(mCacheKey), any(AtomicBoolean.class)))
        .thenReturn(Task.<EncodedImage>forResult(null));
  }

  private void setupDiskCacheGetFailure(com.facebook.imagepipeline.cache.BufferedDiskCache bufferedDiskCache) {
    when(bufferedDiskCache.get(eq(mCacheKey), any(AtomicBoolean.class)))
