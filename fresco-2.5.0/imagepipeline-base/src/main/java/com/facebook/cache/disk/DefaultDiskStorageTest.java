...
public class DefaultDiskStorageTest {
  ...
  private com.facebook.common.internal.Supplier<DefaultDiskStorage> getStorageSupplier(final int version) {
  ...
  private static com.facebook.binaryresource.FileBinaryResource writeToStorage(final DefaultDiskStorage storage, final String resourceId, final byte[] value) throws IOException
  {
    DiskStorage.Inserter inserter = storage.insert(resourceId, null);
  ...
  private static File writeFileToStorage(DefaultDiskStorage storage, String resourceId, byte[] value) throws IOException
  {
    return writeToStorage(storage, resourceId, value).getFile();
  }

  private static File write(DefaultDiskStorage storage, String resourceId, byte[] content) throws IOException
  {
    DiskStorage.Inserter inserter = storage.insert(resourceId, null);
  ...
  private static void writeToResource(DiskStorage.Inserter inserter, final byte[] content) throws IOException
  {
    inserter.writeData(
        new WriterCallback() {
          @Override
  ...
  private void findNewFiles(File directory, Set<File> existing, boolean recurse, List<File> result) {
   ...
   * @param storage
   */
  private static List<DefaultDiskStorage.EntryImpl> retrieveEntries(DefaultDiskStorage storage) throws IOException
  {
    List<DiskStorage.Entry> entries = new ArrayList<>(storage.getEntries());
