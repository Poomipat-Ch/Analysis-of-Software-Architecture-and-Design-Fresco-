...
public class DiskStorageCacheTest {
  ...
  private com.facebook.common.time.SystemClock mClock;

  private com.facebook.imagepipeline.testing.TestExecutorService mBackgroundExecutor;
  ...
  private com.facebook.binaryresource.BinaryResource getResource(DiskStorage storage, final com.facebook.cache.common.CacheKey key) throws IOException {
    return storage.getResource(CacheKeyUtil.getFirstResourceId(key), key);
  }

  private com.facebook.binaryresource.BinaryResource getResource(final com.facebook.cache.common.CacheKey key) throws IOException {
    return mStorage.getResource(CacheKeyUtil.getFirstResourceId(key), key);
  }

  private byte[] getContents(com.facebook.binaryresource.BinaryResource resource) throws IOException {
  ...
  private Thread runInsertionInSeparateThread(final com.facebook.cache.common.CacheKey key, final com.facebook.cache.common.WriterCallback callback) {
    Runnable runnable =
        new Runnable() {
  ...
  private com.facebook.cache.common.CacheKey putOneThingInCache() throws IOException {
    return putOneThingInCache(mCache);
  }

  private com.facebook.cache.common.CacheKey putOneThingInCache(DiskStorageCache cache) throws IOException {
    CacheKey key = new SimpleCacheKey("foo");
    byte[] value1 = new byte[101];
    value1[80] = 'c';
  ...
  private void verifyListenerOnHit(com.facebook.cache.common.CacheKey key, String resourceId) {
  ...
  private void verifyListenerOnMiss(com.facebook.cache.common.CacheKey key) {
    ArgumentCaptor<CacheEvent> cacheEventCaptor = ArgumentCaptor.forClass(CacheEvent.class);
  ...
  private void verifyListenerOnWriteAttempt(com.facebook.cache.common.CacheKey key) {
    ArgumentCaptor<CacheEvent> cacheEventCaptor = ArgumentCaptor.forClass(CacheEvent.class);
  ...
  private String verifyListenerOnWriteSuccessAndGetResourceId(com.facebook.cache.common.CacheKey key, long itemSize) {
    ArgumentCaptor<CacheEvent> cacheEventCaptor = ArgumentCaptor.forClass(CacheEvent.class);
    mCacheEventListenerInOrder
        .verify(mCacheEventListener)
  ...
  private void verifyListenerOnWriteException(com.facebook.cache.common.CacheKey key, IOException exception) {
    ArgumentCaptor<CacheEvent> cacheEventCaptor = ArgumentCaptor.forClass(CacheEvent.class);
    mCacheEventListenerInOrder
        .verify(mCacheEventListener)
  ...
  private void verifyListenerOnReadException(com.facebook.cache.common.CacheKey key, IOException exception) {
    ArgumentCaptor<CacheEvent> cacheEventCaptor = ArgumentCaptor.forClass(CacheEvent.class);
    mCacheEventListenerInOrder
        .verify(mCacheEventListener)
  ...
  private void verifyListenerOnEviction(String[] resourceIds, long[] itemSizes, com.facebook.cache.common.CacheEventListener.EvictionReason reason, long cacheSizeBeforeEviction) {
    int numberItems = resourceIds.length;
    ArgumentCaptor<CacheEvent> cacheEventCaptor = ArgumentCaptor.forClass(CacheEvent.class);
    mCacheEventListenerInOrder
        .verify(mCacheEventListener, times(numberItems))
  ...
  private static class DiskStorageWithReadFailures extends DynamicDefaultDiskStorage {
    ...
    public DiskStorageWithReadFailures(int version, com.facebook.common.internal.Supplier<File> baseDirectoryPathSupplier, String baseDirectoryName, com.facebook.cache.common.CacheErrorLogger cacheErrorLogger) {
  ...
  private static class DuplicatingCacheEventListener implements com.facebook.cache.common.CacheEventListener {
    private final com.facebook.cache.common.CacheEventListener mRecipientListener;

    public DuplicatingCacheEventListener(com.facebook.cache.common.CacheEventListener recipientListener) {
      mRecipientListener = recipientListener;
    }

    @Override
    public void onHit(com.facebook.cache.common.CacheEvent cacheEvent) {
      mRecipientListener.onHit(duplicateEvent(cacheEvent));
    }

    @Override
    public void onMiss(com.facebook.cache.common.CacheEvent cacheEvent) {
      mRecipientListener.onMiss(duplicateEvent(cacheEvent));
    }

    @Override
    public void onWriteAttempt(com.facebook.cache.common.CacheEvent cacheEvent) {
      mRecipientListener.onWriteAttempt(duplicateEvent(cacheEvent));
    }

    @Override
    public void onWriteSuccess(com.facebook.cache.common.CacheEvent cacheEvent) {
      mRecipientListener.onWriteSuccess(duplicateEvent(cacheEvent));
    }

    @Override
    public void onReadException(com.facebook.cache.common.CacheEvent cacheEvent) {
      mRecipientListener.onReadException(duplicateEvent(cacheEvent));
    }

    @Override
    public void onWriteException(com.facebook.cache.common.CacheEvent cacheEvent) {
      mRecipientListener.onWriteException(duplicateEvent(cacheEvent));
    }

    @Override
    public void onEviction(com.facebook.cache.common.CacheEvent cacheEvent) {
    ...
    private static com.facebook.cache.common.CacheEvent duplicateEvent(com.facebook.cache.common.CacheEvent cacheEvent)
    {
